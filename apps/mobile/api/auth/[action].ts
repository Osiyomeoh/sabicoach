...
async function ensureSchema(): Promise<void> { if (!pool) return; await pool.query("CREATE EXTENSION IF NOT EXISTS \\"pgcrypto\\"; CREATE TABLE IF NOT EXISTS students (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), display_name TEXT NOT NULL, exam_type TEXT NOT NULL DEFAULT 'JAMB', target_score INTEGER NOT NULL DEFAULT 280, current_score INTEGER NOT NULL DEFAULT 234, created_at TIMESTAMPTZ NOT NULL DEFAULT now()); ALTER TABLE students ADD COLUMN IF NOT EXISTS email TEXT; ALTER TABLE students ADD COLUMN IF NOT EXISTS password_hash TEXT; CREATE UNIQUE INDEX IF NOT EXISTS students_email_unique ON students (email) WHERE email IS NOT NULL; CREATE TABLE IF NOT EXISTS sessions (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), student_id UUID NOT NULL REFERENCES students(id) ON DELETE CASCADE, token_hash TEXT NOT NULL UNIQUE, expires_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now());"); }
...
export default async function handler(request: Request, response: Response): Promise<void> {
  ...
  try {
    ...
    if (action === "signup" && request.method === "POST") {
      ...
      const created = await pool.query<Student>("INSERT INTO students (display_name, email, password_hash) VALUES ($1,$2,$3) RETURNING id, display_name, exam_type, target_score, current_score", [displayName, email, await passwordHash(password)]);
      const token = randomBytes(32).toString("base64url"); await pool.query("INSERT INTO sessions (student_id, token_hash, expires_at) VALUES ($1,$2,now() + interval '30 days')", [created.rows[0].id, tokenHash(token)]);
    ...
    if (action === "login" && request.method === "POST") {
      ...
      const token = randomBytes(32).toString("base64url"); await pool.query("INSERT INTO sessions (student_id, token_hash, expires_at) VALUES ($1,$2,now() + interval '30 days')", [student.id, tokenHash(token)]);
      response.setHeader("Set-Cookie", sessionCookie(token)); response.status(200).json({ student: publicStudent(student) }); return;
    }
    response.status(404).json({ error: "Not found." });
  } catch { response.status(500).json({ error: "We could not complete this request. Please try again." }); }
}
